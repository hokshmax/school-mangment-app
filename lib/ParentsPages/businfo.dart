...
class busViewState extends State<busView> {
  ...
  getNationalID()
  async {
    await  FirebaseFirestore.instance.collection("pernatsAccount").doc(id).get().then((value) {

      setState(() {
        studentid=value.get("nationalId").toString();

      });
      FirebaseFirestore.instance.collection("Students").doc(studentid).get().then((value22) {
        setState(() {
          busid=value22.get("busId").toString();
...
//backgroundColor: Colors.white,
        ...
        body:FutureBuilder(
          future: FirebaseFirestore.instance.collection("Students").where("parentNID",isEqualTo:studentid).get(),
          builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) =>snapshot.hasData? StreamBuilder(
              stream:  FirebaseFirestore.instance.collection("Bus").where("busID",isEqualTo: snapshot.data['busId']).snapshots(),
                                            ...
                                            child: Column(
                                                ...
                                                children:<Widget> [
                                                  ...
                                                  Text(snap.data.docs.elementAt(index)
                                                      .data()['BusPlateNumber'].toString(),style: TextStyle(
                                                  ...
                                                  SizedBox(height: 5,),
                                                  Text(snap.data.docs.elementAt(index)
                                                      .data()['Capcity'].toString(),style: TextStyle(
                                                      fontSize: 24, color: Colors.black
                                                  ),),
                                                  SizedBox(height: 20,),
                                                  Row(
