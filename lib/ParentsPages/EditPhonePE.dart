...
class editphonePE extends StatefulWidget{
  int index;
  editphonePE({this.index});
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return editphonePEState(index: this.index);
  }

}

class editphonePEState extends State<editphonePE>{
  int index;
  TextEditingController getText=TextEditingController();

  editphonePEState({this.index});
  ...
  getNationalID() async {
    await FirebaseFirestore.instance.collection("pernatsAccount").doc(myid)
        .get()
        .then((value) {
      setState(() {
...
//backgroundColor: Colors.white,
        ...
        body: StreamBuilder(
        stream:  FirebaseFirestore.instance.collection("pernatsAccount").where("nationalId",isEqualTo: nationalId).snapshots(),
    ...
    child: Column(
    children: [
      Center(child: Text('Update Mobile Number',style: TextStyle(fontSize: 24, color: Colors.amber,fontWeight: FontWeight.w600))),
      SizedBox(height: 20,),

      Text('Please enter your new Mobile Number',style: TextStyle(fontSize: 18, color: Colors.black,)),
      SizedBox(height: 20,),
    TextField(
    controller: getText,
        ...
        .data()['phone'].toString(),
                      ...
                      Padding(
                        ...
                        child: ElevatedButton(onPressed: () async {
                          ...
                          //     "pernatsAccount").doc(
                          //     myid).update({
                          //   "phone": getText.value.text});

                          server().perantChangePhone(newphone: getText.value.text);
                          /*  if(index!=0)
                                     {
